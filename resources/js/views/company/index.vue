        ...
        <div class="px-4 py-5  sm:px-6 flex justify-between items-center">
          ...
          <div class="overflow-x-auto scrollbar-color">
            <table class="min-w-full whitespace-nowrap align-middle text-sm ">
              ...
              <tbody>
                <template v-for="(company, companyIndex) in this.companies" :key="company.id">
                  <tr class="even:bg-gray-50 dark:even:bg-gray-900/50">
                    <td class="p-3 text-center">{{ companyIndex + 1 }}</td>
                    <td class="p-3 text-center">   <div class="w-full pl-6">
                    ...
                    <td class=" text-center ">
                      ...
                      <span v-if="permissions.includes(`edit-${small}`)" class="bg-blue-400 p-1 text-white border rounded border-blue-500 mr-2 hover:bg-blue-600 transition-colors duration-300">
                        <a @click.prevent="edit(company.id)" href="#">
                          <svg class="icon-edit h-5 w-5 " fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24">
                      ...
                      <span v-if="permissions.includes(`delete-${small}`)" class="bg-red-500 p-1 border rounded border-red-500 text-white hover:bg-red-600 transition-colors duration-300">
                        <a @click.prevent="deleteRole(company.id)" href="#">
                          <svg class="icon-delete h-5 w-5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24">
        ...
        <tr>
          <td colspan="6">
            <table class="min-w-full ml-12 whitespace-nowrap align-middle text-sm border ">
              ...
              <tbody>
                <tr v-for="(store, storeIndex) in company.stores" :key="storeIndex" class="even:bg-gray-300 dark:even:bg-gray-900/50">
                  <td class="p-3 text-center">{{ storeIndex + 1 }}</td>
                  <td class="p-3 text-center">  <div class="w-full pl-6">
                    <img :src="getImagePathstore(store.img)" class="shadow-xl h-10  rounded-md" />
                </div></td>
