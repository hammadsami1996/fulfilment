    ...
    <div>
        <div class="px-4 py-5  sm:px-6 flex justify-between items-center">
            ...
            <div class="mt-3 pb-4 sm:mt-0 sm:ml-4 flex justify-end">
                <router-link :to="{name:`create-${small}`}" type="button" class="inline-flex justify-center items-center space-x-2 border font-semibold rounded-lg px-3 py-2 leading-5 text-sm border-gray-200 bg-blue-400 text-white">
                    Create
                </router-link>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                columns: [
                    ...
                    {label: 'Date', field: 'voucher_date'},
                    // {label: 'Other Account', field: 'accounttitle' , displayText:'account'},
                    {label: 'Remarks', field: 'detail_remarks',},
        ...
        methods: {
            edit(id) {
                this.$router.push(`${this.small}/${id}/edit`)
            },
            deleteRole(e) {
                byMethod('delete', `/api/journal_voucher/${e}`)
                    .then((res) => {
                        if (res.data.deleted) {
